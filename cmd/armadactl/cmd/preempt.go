...
func preemptExecutorCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return a.PreemptOnExecutor(onExecutor, queues, priorityClasses, pools)
		},
	}
...
func preemptNodeCmd() *cobra.Command {
	...
	cmd.Flags().StringSliceP(
		...
		"q",
		[]string{},
		"Preempt jobs on node for specified executor matching the specified queue names. If no queues are provided, jobs across all queues will be preempted. Provided queues should be comma separated, as in the following example: queueA,queueB,queueC.",
	)
	cmd.Flags().StringSliceP(
		"priority-classes",
		"p",
